...
- `/空间状态` 的「上次生成依据」会说明这次生成用了哪个人格、有没有引用日程、
  有没有使用联网素材与聊天记录，供排查时核对。
...
主动消息（早安、晚安、节日祝福）默认需要对方同意，由 `active_msg_require_optin` 控制，
**早安、晚安、节日祝福的收件人口径完全一致**：

- `active_msg_require_optin` **开启**（默认）时，收件人 = **已在 `/私聊开` 接受过、
  且没有单独关掉该项的用户**（早安看「早安」开关、晚安看「晚安」开关、节日祝福看「节日」开关）；
  此时**不读 `greet_users`**。
- `active_msg_require_optin` **关闭**时，退回按 `greet_users` 发送，不检查偏好。
- 用户第一次私聊机器人时会收到一条简短说明（其中写明不回应视为不接受），
  可用 `/私聊开` 或 `/私聊关` 开启、关闭，也可以只开或只关某一项。
...
收件人口径见上文「主动消息同意」。是否先转草稿确认沿用 `draft_for_greet`。
...
│   ├── interact.py            说说互动（巡检、去重、点赞、AI 评论、回复评论）
