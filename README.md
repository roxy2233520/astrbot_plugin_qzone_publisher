...
[![Tests](https://img.shields.io/badge/tests-719%20passed-2ea44f)](tests/)
...
| 回复评论 | 自己说说下有人评论时，用 AI 回一句话（`interact_reply_enabled`，**默认关闭**）：只回别人的评论、同一条评论只回一次、每轮总数与每条说说都有上限。**回复一律直接发出，不经过草稿确认**，并使用独立的巡检任务（`interact_reply_cron`，默认只在中午与晚上两个时段每 30 分钟一轮、窗口 7 天），不再等一天一次的互动巡检 |
...
| 回复报错说登录态失效了 | 先看日志里的原因：写「返回的是页面」或「请求被拒绝（403）」属于接口地址或参数问题，升级插件版本即可，**不必重取登录态**；只有写「登录态可能已失效」时才需要用 `/空间重登` 重取一次 |
...
- **回复失败的原因会分开讲**：接口返回的是页面而不是数据（例如拿到的是一段 JS 框架页）
  记作「返回页面」，被判为 403 记作「请求被拒绝」，这两种都**不会**去重取登录态，
  说明接口地址或参数不被接受，升级插件版本后重试即可；只有确实返回登录页或者 HTTP 401 时
  才判定为**登录态失效**，此时插件会自动重取登录态并重试一次，仍失败才提示用 `/空间重登`。
  失败日志里会一并写出接口地址、topicId、commentId、commentUin 与响应正文片段，
  便于对照排查。
- **评论 id 会先校验再用**：空 id、与说说 id 相同、以及只有几位数字的明显异常 id
  都会被判为非评论（多由界面结构变化或解析偏差产生），当轮跳过并写明原因，
  不会把无效请求发到接口上。
