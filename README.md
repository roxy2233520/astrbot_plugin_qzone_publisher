...
[![Tests](https://img.shields.io/badge/tests-762%20passed-2ea44f)](tests/)
...
- **未确认的回复不会重复发**：回复请求发出后**回查确认**——只要在该评论（或子回复）下
  发现任何一条来自我的回复就算成功（正文略有差异也算，正文一致与否只写进日志）；
  如果回查没确认（例如接口真的没发出、或详情还没刷新），会写一条
  `<数据目录>/replied_attempts.json` 尝试记录，**24 小时内不再对这条候选发起回复**
  （超过 24 小时才允许再试一次），并在 `/空间回复`、`/空间状态` 里提示
  「有 N 条回复发出后没回查确认」，需要人工到空间里确认。
  这样即使遇到「其实发出去了却查不到」的情况，也只会多出一条，不会每轮叠加。
...
├── replied_attempts.json  发出后没回查确认的回复（24 小时内不再重发）
