...
| 说说互动 | 定时读取关注 QQ 号**最近 N 天内最新的一条**说说（`interact_days`，**默认只读**）；最新一条超出窗口就整体跳过，不去评论几天前的老说说。可选自动点赞与 AI 评论，按 `uin_tid` 去重；**只有特权名单里的人、或接受过主动消息的人才会被自动点赞 / 评论**（并集口径见下文「主动消息同意」） |
| 回复评论 | 自己说说下有人评论时，用 AI 回一句话（`interact_reply_enabled`，**默认关闭**）：只回别人的评论、同一条评论只回一次、每轮总数与每条说说都有上限。**回复一律直接发出，不经过草稿确认**，是否成功以**回查评论详情**确认（回复接口成功时也只返回页面）；使用独立的巡检任务（`interact_reply_cron`，默认只在中午与晚上两个时段每 30 分钟一轮、窗口 7 天），不再等一天一次的互动巡检 |
...
| 回复报错说没成功 | 先看日志里的回查结果：回复接口成功时也只回页面，所以插件以「回查评论详情能否找到自己的回复」为准。写「无法确认」表示这次没在评论下查到，可稍后在空间里确认；写「返回的是页面」「请求被拒绝（403）」属于接口地址或参数问题，升级插件版本即可，**不必重取登录态**；只有写「登录态可能已失效」时才需要用 `/空间重登` 重取一次 |
...
- **互动口径（并集）**：好友互动的自动点赞 / AI 评论与「评论回复」共用一条口径，
  由 `interact_reply_require_optin`（**默认开启**）控制：
  **特权名单**（`interact_reply_uins`，面板里叫「特权名单」）里的人**无需任何同意**，
  既可被自动评论 / 回复他的说说，也会被回复他在自己说说下的评论与子回复；
  名单外但**接受过主动消息**（用过 `/私聊开`）的人同样可以；
  **两者都不是的人不互动**——不点赞、不评论他的说说，也不回复他的评论，
  跳过时会在日志与巡检汇总里写明（`gated` 人数）。
  关闭这个开关即退回「谁都可以互动」的旧行为。
  `/空间回复` 与 `/空间状态` 会显示「回复对象：特权名单 N 人 ∪ 已同意 M 人（两者都不是则不互动）」。
...
- **已经在评论下回复过就跳过，但只针对具体那一条**：接口会把一条评论下的回复放在
  ...
  **明确指向这一条**的我的回复（我的回复的 `parent_tid` 等于这条的 id，且时间不早于这条），
  有就跳过并在日志里说明。线程里别处有我的回复不会牵连其它候选，所以**对方又回了新的
  那条时，会继续回应对方最新的回复**；历史上已经重复发出的回复也能被这条保护认出来。
- **空间里父评论与子回复各自独立编号**（都从 1 开始），所以插件按「**评论 / 子回复层级**」
  精确去重，不用裸 tid 数字：
  顶层评论的键是 `{说说tid}_c{评论tid}`，子回复是 `{说说tid}_c{父评论tid}_r{子回复tid}`
  （更深一层继续拼 `_r{tid}`）；日志里的候选标识同样带层级（`c1`、`c1/r2`），
  不会出现两条都叫 `1` 分不清的情况。老版本写下的无层级键仍能读（只用于顶层评论）。
- **候选最新优先**：一条评论下的候选（父评论 + 每一层子回复）按时间倒序处理，
...
- **未确认的回复不会重复发**：回复请求发出后**回查确认**——按**线程顶层评论**
  ...
  **24 小时内不再对这条候选发起回复**（超过 24 小时才允许再试一次），
  并在 `/空间回复`、`/空间状态` 里提示「有 N 条回复发出后没回查确认」，
  需要人工到空间里确认。这样即使遇到「其实发出去了却查不到」的情况，
  也只会多出一条，不会每轮叠加。
...
- **谁能被互动：特权名单 ∪ 已同意**：`interact_reply_uins`（面板里叫「特权名单」）
  填 QQ 号后，这些人**无需任何设置**就会被自动评论 / 回复（含他的说说，以及他在自己
  说说下的评论与子回复）；名单外的人只要用过 `/私聊开`（接受过主动消息）同样会被回复；
  **两者都不是的人不互动**。这套口径同时管住「好友互动的自动点赞 / 评论」那条路径。
  想退回「谁都可以互动」就关掉 `interact_reply_require_optin`（默认开启）。
- **每轮都有日志，逐条写清处置**：无论有没有新评论，都会写一行
  `[reply] 第 N 轮评论巡检：检查 … 回复 … 跳过 …`，并**对每条候选写一行结论**，
  例如 `[reply] 说说tid/候选tid：跳过（已回复）`、`跳过（已尝试未确认，24 小时内不重发）`、
  `跳过（不在特权名单且未同意）`、`已回复（回查命中）`、`失败（保留尝试记录）`，
  一眼就能看出没有重复发。`/空间回复` 会显示开关、巡检间隔、时间窗口、回复方式、
  每轮上限、今日已回、**回复对象口径**、**未确认条数**与下次巡检。
...
- 回复评论不产生草稿（巡检到即直接发出，并以回查评论详情确认是否成功），草稿箱里只会有说说、评论与问候草稿；
...
├── replied_comments.json  已回复过的评论（键带评论/子回复层级）
├── replied_attempts.json  发出后没回查确认的回复（24 小时内不再重发，键同样带层级）
